...
public class LoginController implements ActionListener, MetodosPublicos {
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getSource() == this.lg.bIngresar) {
            String user = this.lg.getId();
            String contrasena = this.lg.getPassword();
            if (!user.isEmpty() && !contrasena.isEmpty()) {
                if (user.matches("\\d{10}") && contrasena.matches("(?=.{8,10}$)[a-zA-Z0-9]+([/$#?!%][a-zA-Z0-9]*)*")) {
                    JOptionPane.showMessageDialog(lg, "iniciaste sesion");
                } else {
                    if (Pattern.compile("[^0-9]").matcher(user).find()) {
            ...
            } else if (user.isEmpty() && contrasena.isEmpty()) {
